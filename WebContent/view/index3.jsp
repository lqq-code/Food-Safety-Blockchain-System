...
<table width="100%" border="2" >
         ...
         <%
         	int ki=10;
         	int kj=7;
         	String sfo[][]=new String[ki][kj];
			AssetsDao da=new AssetsDao();
			BlockDao dao=new BlockDao();
			String name=session.getAttribute("name").toString();
			String b[]=da.query(name,20);
			String ta;
			for(int i=0;i<ki&&i<b.length;i++){
				ta=b[i];
				int day=dao.findlast(ta);
				String laststep=dao.find(ta,day).data;
				String lasttime=dao.find(ta,day).timeStamp;
				int alstep=dao.allstep(ta);
				double av=(double)alstep/day;
	        	String m = new java.text.DecimalFormat("#0.00").format(av);
	        	boolean isB=dao.isChainValid(ta);
				sfo[i][0]=ta;
				sfo[i][1]=laststep;
				sfo[i][2]=lasttime.substring(0,16);
				sfo[i][3]=String.valueOf(day);
				sfo[i][4]=String.valueOf(alstep);
				sfo[i][5]=m;
				sfo[i][6]=String.valueOf(isB);
				for(int o=0;o<7;o++){
					System.out.println(sfo[i][o]);
				}
				
				
			}
			request.setAttribute("sfo",sfo);
		%>
         ...
         for(int i=0;i<10;i++){
        	 if(b[i]!="0"){
	    		 ...
	    		 out.print(""+"<a href=\"chartjs.jsp?tname="+sfo[i][0]+"\">"+sfo[i][0]+"</a>" );
        		 out.print("</td>");
        		 //鸡 当天步数
        		 out.print("<td>");
        		 out.print(sfo[i][1]);
        		 out.print("<br>(<font size='1' color='red'> "+sfo[i][2]+"</font>)");
        		 out.print("</td>");	 
	        	 //鸡计步天数
	        	 out.print("<td >");
	        	 out.print(sfo[i][3]);
